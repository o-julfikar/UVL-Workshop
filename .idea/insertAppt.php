<?php
    ...
    if (mysqli_num_rows($result_session) > 0) {
        ...
        if ($days > 30) {
            ...
            $sql_get_appt = "SELECT * FROM car_has_appointment_with_mechanic " .
                            "INNER JOIN mechanic " .
                            "ON mechanic.mechanic_id = car_has_appointment_with_mechanic.mechanic_id " .
                            "WHERE car_license_number = '$license' AND appointment_date = '$apptDate'";
            $result_appt = mysqli_query($conn, $sql_get_appt);
            if (mysqli_num_rows($result_appt) > 0) {
                $rowappt = mysqli_fetch_assoc($result_appt);
                $mechanic_name = $rowappt["mechanic_name"];
                echo "APPTEXIST" . "\n" . $mechanic_name;
                return;
            }
